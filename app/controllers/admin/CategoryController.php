...
class CategoryController extends AppController{
    ...
    public function deleteAction(){
        ...
        $errors = '';
        if($children){
            $errors .= '<li>Видалення неможливо, у категорії є вкладені категорії</li>';
        }
        $products = \R::count('product', 'category_id = ?', [$id]);
        if($products){
            $errors .= '<li>Видалення неможливо, у категорії є товари</li>';
        }
        if($errors){
    ...
    public function addAction(){
        if(!empty($_POST)){
            ...
            if($id = $category->save('category')){
                ...
                $cat->alias = $alias;
                \R::store($cat);
                $_SESSION['success'] = 'Категорія додана';
            }
            redirect();
        }
